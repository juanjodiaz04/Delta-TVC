...
void set_dirs_mat(bool change){
    if(change==false){//modo escaneo
        gpio_set_dir_in_masked(MAT_READ_PINS); //Lee pines, por ej solo filas
        gpio_set_dir_out_masked(MAT_WRITE_PINS);//Pines de Columnas para cerrar circuito
        //for (uint8_t i=6;i<=9;i++){gpio_pull_down(i);}
    } else{//modo scanning
        gpio_set_dir_in_masked(MAT_WRITE_PINS); // Ahora se revisan los pines de columnas
        gpio_set_dir_out_masked(MAT_READ_PINS); // Pines de filas para cerrar circuito
...
bool read_mat(volatile uint8_t* value){
    ...
    */
    ...
    if(reading==1){// Ha recibido una interrupción, pasa a modo scanning
        ...
        uint8_t lectura=(gpio_get_all() & MAT_WRITE_PINS)>>2; //Aplica máscara y desplaza para pines de interés
        if (lectura){
            uint8_t c=0;
